...
class LightningModel(L.LightningModule):
    ...
    def _shared_step(self, batch):
        ...
        predicted_y = self(features)

        loss = F.mse_loss(predicted_y, true_y)
        return loss, true_y, predicted_y

    def training_step(self, batch, batch_idx):
        loss, true_y, predicted_y = self._shared_step(batch)

        self.log("train_loss", loss)
        self.train_mse(predicted_y, true_y)
        self.log(
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.val_mse(predicted_y, true_y)
        self.log("val_mse", self.val_mse, prog_bar=True)

    def test_step(self, batch, batch_idx):
        _, true_y, predicted_y = self._shared_step(batch)
        self.test_mse(predicted_y, true_y)
